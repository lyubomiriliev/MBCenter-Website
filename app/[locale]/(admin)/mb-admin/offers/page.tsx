...
import { getSearchVariants } from "@/hooks/useOffers";
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      if (filters.search) {
        ...
        const vinLicenseConditions = variants.flatMap((v: string) => [
          `vin_text.ilike.%${v}%`,
          `license_plate.ilike.%${v}%`,
        ]);
        query = query.or([...baseConditions, ...vinLicenseConditions].join(","));
      }
      if (filters.dateFrom) {
      ...
      for (const offer of offers as OfferWithRelations[]) {
        try {
          ...
          const pdfDoc = (
            <OfferPDFv3 offer={offer} locale={locale as "bg" | "en"} />
          );
          const blob = await pdf(pdfDoc).toBlob();

          // Add to ZIP with safe filename
          const fileName = `Offer_${offer.offer_number}_${
            offer.customer_name || "Unknown"
          }.pdf`.replace(/[^a-zA-Z0-9_\-\.]/g, "_");
          folder?.file(fileName, blob);
        } catch (err) {
          console.error(
            `Error generating PDF for offer ${offer.offer_number}:`,
            err
          );
        }
      }
      ...
      link.href = url;
      link.download = `MBCenter_Offers_${
