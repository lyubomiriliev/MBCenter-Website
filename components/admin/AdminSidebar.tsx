...
const navItems: NavItem[] = [
  ...
  {
    href: "/create-check",
    labelKey: "admin.sidebar.createCheck",
    icon: (
      <svg
        className="w-5 h-5"
        fill="none"
  ...
  {
    href: "/earnings",
    labelKey: "admin.sidebar.earnings",
    superAdminOnly: true,
    icon: (
    ...
    <>
      ...
      >
        ...
        <div className="p-4 border-b border-mb-border flex items-center justify-between shrink-0">
          <Link
            href={`/${locale}/mb-admin/offers`}
            className="block relative w-60 h-[54px]"
            onClick={close}
          >
            <Image
        ...
        <div className="p-4 border-b border-mb-border">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                {profile?.role === "reception" ? t("admin.roles.reception") : profile?.role === "admin" ? t("admin.roles.admin") : t("admin.roles.mechanic")}
              </Badge>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto min-h-0">
          {navItems.map((item) => {
            // Hide super-admin-only items (e.g. Settings) from reception
            ...
            return (
              <Link
                ...
                href={href}
                onClick={() => {
                  if (item.href === "/offers") clearOfferFilters();
                  close();
                }}
                className={cn(
